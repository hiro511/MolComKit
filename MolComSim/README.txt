	...
	C)  The following parameter name, value format, and default value triplets are valid:
		...
		l) For movement in the Y dimension this is:
 		...
 		m) For movement in the Z dimension this is:
  		...
  		n) The user can specify the size of molecule(s), how many molecules to send out each time,
  			...
  			default values: 1 1 INFO ACTIVE 0
  						1	1 ACK ACTIVE 0
  		...
  		u) There is an option that, when there is a collision between an acknowledgement molecule
			...
			v) Probability of collisions is used to specify how likely (continuous range
  			from 0 to 1) molecules collide with each other.
  			Name: probCollisions, format: double (between 0 and 1), default value: 1.0
			w) The assembling parameter is specified whether or not the receiver(s) assembles packets into a message.
			  Name: assembling, format integer (0 = false, 1 = true), default value: 0
			x) The number of required packets to be used in the simulation can be specified using:
				Name: numRequiredPackets, format: integer, default value: 1
			y) FEC is used to specify a type of forward error correction.  Each type requires different parameter(s).
			  Name: FEC,
				format (space-separated value(s)):
				<type of FEC as NONE or PARITYCHECK>
				When the type = NONE, any parameters isn't needed.
				When the type = PARITYCHECK, a parity codes rate is required.
				<rate as double>
